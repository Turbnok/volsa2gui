...
ipcMain.handle("checkvolsa", async () => {
  return await execShellCommand(
    `command -v ${join(app.getPath("home"), "/.cargo/bin/volsa2-cli")}`
  )
    .then((list: string) => {
      return true;
    })
...
ipcMain.handle("list", async () => {
  return await execShellCommand(
    `${join(app.getPath("home"), "/.cargo/bin/volsa2-cli")} list -a`
  )
    .then((list: string) => {
      ...
      return {
        ...
        samples: samples.map((item) => {
          let r: Array<string> | null = item.match(
            /^(.+?): (.+?) - length: (.+?), speed: (.+?), level: (.+)/
          );
...
ipcMain.handle("remove", async (e, id: number) => {
  return await execShellCommand(
    join(app.getPath("home"), `/.cargo/bin/volsa2-cli remove ${id}`)
  )
    .then((result: string) => {
      return "ok";
    })
