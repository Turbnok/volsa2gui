...
<template>
  <div class="row">
    ...
    <div class="name">
      <span class="number">{{
        props.sound.id.toString().padStart(3, "0")
      }}</span>
      : {{ props.sound.changed ? "✨" : ""
      }}{{ props.sound.textNew ? props.sound.textNew : props.sound.text
      }}<span
        :class="props.sound.changed ? 'file new' : 'file'"
        v-if="props.sound.file || props.sound.fileNew"
      >
        ({{ props.sound.fileNew ?? props.sound.file }})</span
      >
    </div>
    ...
    <div class="actions">
      <button
        :disabled="!props.sound.file && !props.sound.fileNew"
        @click="play"
      >
        ♪
      </button>
      ...
      <button
        :disabled="!props.sound.sync || !props.sound.text"
        @click="remove"
      >
        ⨯
      </button>
